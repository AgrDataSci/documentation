...
If participants withdraw when planting materials arrive (e.g. de Boef and Thijssen [-@deBoef2007]) they should be replaced with individuals representing similar social and task groups.

Table 2. Chosen participants in one community in Southwest Nigeria who participated in the tricot evaluation based on a task group approach (using information in Table 1).
...
For the further sake of social differentiation and allowing to segment the dataset later, a [Rural Household Multi-indicator Survey (RHoMIS)](https://www.rhomis.org/) core social questionnaire should be administered to each of the chosen participants.
...
### Making tricot inclusive: Gender and social heterogeneity 
...
* Prior informed consent of no Personally Identifiable Information (PII) publication.
...
* Application of privacy-preserving methods before exporting data. This process can be automated through the automatic detection of potential personally identifiable information (PII), confidential data, or other protected content, resulting in shareable data.
